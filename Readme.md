...
- 데이터 내역 : 최근 1년간의
    ...
    - ![스크린샷 2021-05-16 오후 11.54.57](Readme.assets/dataframe.png)
- 분석 절차
    - 지역별 매물의
...
![스크린샷 2021-05-16 오후 11.47.15](Readme.assets/csv.png)

- 생성한 것을 바탕으로 지도에 표시(빨간색 : 단독주택 / 녹색 : 연립주택 / 파란색 : 오피스텔)![스크린샷 2021-05-17 오전 12.33.34](Readme.assets/house_classify.png)
...
- raw값과 설문데이터에서의 유저 선호도를 종합하여 아래의 수식에 의거해서 값을 계산한다.
  ...
  ![스크린샷 2021-05-17 오전 12.42.04](Readme.assets/formula.png)
  - C : 유저가 정의한 가중치 , V : 정규화를 통해 추출한 값

- 이렇게 정규화한 값에 100을 곱해주어, 최종 평점을 0~100 사이에 위치하게 함.

![스크린샷 2021-05-17 오전 12.43.27](Readme.assets/normalize.png)
...
![스크린샷 2021-05-17 오전 12.44.48](Readme.assets/visualize.png)

## 결과 해석

- 결과 측정을 위해 세 명의 서로 다른 가상의 인물과 특징을 설정한 뒤, 위에서 분석한 데이터를 통해 어느 지역에 거주하는 것이 합리적인지를 표시하고자 함.
- 각 인물들의 설정은 아래의 그림과 같음.![스크린샷 2021-05-17 오전 12.45.44](Readme.assets/status.png)

- 각 인물들의 설정에 맞게 유저 선호도를 입력한 뒤, 위의 평점 계산식을 활용하여 시각화함.

![스크린샷 2021-05-17 오전 12.46.15](Readme.assets/character.png)
