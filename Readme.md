...
1. 각 지역별 평균 계약 월세 / 보증금과 계약 건수 확보(성공!)
2. 각 지역별 좌표 '전북 전주시'+' '+dong+' '+도로명 으로 API 조회한 뒤 좌표 확인해서 데이터셋에 추가(성공!)![estate](Readme.assets/estate.PNG)
3. 주택 포인트별로 해당 좌표에 해당하는 월세/보증금을 합산한 뒤 csv 문서를 생성할 것.
